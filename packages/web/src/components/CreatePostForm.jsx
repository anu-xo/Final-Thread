      ...
      <div className="flex gap-2">
        {['text', 'link'].map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => register('postType').onChange({ target: { value: t, name: 'postType' } })}
            className={`px-3 py-1 rounded-md text-sm border ${postType === t ? 'bg-blue-600 text-white' : 'bg-white'}`}
          >
